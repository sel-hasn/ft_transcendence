...
CREATE TABLE IF NOT EXISTS tournament_participants (
    ...
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    UNIQUE(tournament_id, user_id)  -- User can only join once per tournament
);

-- Friends system (Simple version)
...
CREATE TABLE IF NOT EXISTS chat_channel_members (
    ...
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    UNIQUE(channel_id, user_id)  -- User can only be member once per channel
);
